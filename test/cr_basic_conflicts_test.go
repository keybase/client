...
func TestCrConflictWriteFile(t *testing.T) {
	test(t,
		...
		as(bob, noSync(),
			write("a/b", "uh oh"),
			reenableUpdates(),
			lsdir("a/", m{"b$": "FILE", "b.conflict.bob.unknown.0001-01-01": "FILE"}),
			read("a/b", "world"),
			read("a/b.conflict.bob.unknown.0001-01-01", "uh oh"),
		),
		as(alice,
			lsdir("a/", m{"b$": "FILE", "b.conflict.bob.unknown.0001-01-01": "FILE"}),
			read("a/b", "world"),
...
func TestCrConflictSetattrVsRecreatedFileInRoot(t *testing.T) {
	test(t,
		...
		as(bob, noSync(),
			...
			mkfile("a", "world"),
			reenableUpdates(),
			lsdir("", m{"a$": "EXEC", "a.conflict.bob.unknown.0001-01-01": "FILE"}),
			read("a", "hello"),
			read("a.conflict.bob.unknown.0001-01-01", "world"),
		),
		as(alice,
			lsdir("", m{"a$": "EXEC", "a.conflict.bob.unknown.0001-01-01": "FILE"}),
			read("a", "hello"),
...
func TestCrConflictCauseRenameOfMergedFile(t *testing.T) {
	test(t,
		...
		as(bob, noSync(),
			write("a/b/c", "uh oh"),
			reenableUpdates(),
			lsdir("a/", m{"b$": "DIR", "b.conflict.alice.unknown.0001-01-01": "FILE"}),
			read("a/b.conflict.alice.unknown.0001-01-01", "world"),
			read("a/b/c", "uh oh"),
		),
		as(alice,
			lsdir("a/", m{"b$": "DIR", "b.conflict.alice.unknown.0001-01-01": "FILE"}),
...
func TestCrConflictUnmergedRenameFileOverModifiedFile(t *testing.T) {
	test(t,
		...
		as(bob, noSync(),
			...
			lsdir("a/", m{"b$": "FILE", "b.conflict.bob.unknown.0001-01-01": "FILE"}),
			read("a/b", "uh oh"),
			read("a/b.conflict.bob.unknown.0001-01-01", "world"),
		),
		as(alice,
			lsdir("a/", m{"b$": "FILE", "b.conflict.bob.unknown.0001-01-01": "FILE"}),
			read("a/b", "uh oh"),
...
func TestCrConflictUnmergedRenameDirOverModifiedFile(t *testing.T) {
	test(t,
		...
		as(bob, noSync(),
			...
			lsdir("a/", m{"b$": "DIR", "b.conflict.alice.unknown.0001-01-01": "FILE"}),
			read("a/b/d", "world"),
			read("a/b.conflict.alice.unknown.0001-01-01", "uh oh"),
		),
		as(alice,
			lsdir("a/", m{"b$": "DIR", "b.conflict.alice.unknown.0001-01-01": "FILE"}),
			read("a/b/d", "world"),
...
func TestCrConflictUnmergedRenamedDir(t *testing.T) {
	test(t,
		...
		as(bob, noSync(),
			...
			rename("a/b", "a/d"),
			reenableUpdates(),
			lsdir("a/", m{"d$": "DIR", "d.conflict.bob.unknown.0001-01-01": "DIR"}),
			lsdir("a/d", m{"e": "FILE"}),
			lsdir("a/d.conflict.bob.unknown.0001-01-01", m{"c": "FILE", "f": "FILE"}),
			read("a/d.conflict.bob.unknown.0001-01-01/c", "hello"),
			read("a/d/e", "world"),
			read("a/d.conflict.bob.unknown.0001-01-01/f", "uh oh"),
		),
		as(alice,
			lsdir("a/", m{"d$": "DIR", "d.conflict.bob.unknown.0001-01-01": "DIR"}),
			lsdir("a/d", m{"e": "FILE"}),
...
func TestCrConflictMergedRenamedDir(t *testing.T) {
	test(t,
		...
		as(bob, noSync(),
			...
			read("a/d/c", "hello"),
			read("a/d.conflict.bob.unknown.0001-01-01/e", "world"),
			read("a/d/f", "uh oh"),
		),
		as(alice,
			lsdir("a/", m{"d$": "DIR", "d.conflict.bob.unknown.0001-01-01": "DIR"}),
...
func TestCrConflictMergedRenameDirOverModifiedFile(t *testing.T) {
	test(t,
		...
		as(bob, noSync(),
			write("a/b", "uh oh"),
			reenableUpdates(),
			lsdir("a/", m{"b$": "DIR", "b.conflict.bob.unknown.0001-01-01": "FILE"}),
			read("a/b/d", "world"),
			read("a/b.conflict.bob.unknown.0001-01-01", "uh oh"),
		),
		as(alice,
...
func TestCrConflictUnmergedRenamedDirDouble(t *testing.T) {
	test(t,
		...
		as(bob, noSync(),
			...
			rename("a/b", "a/d"),
			reenableUpdates(),
			lsdir("a/", m{"d$": "DIR", "d.conflict.bob.unknown.0001-01-01$": "DIR", "d.conflict.bob.unknown.0001-01-01_\\(1\\)": "DIR"}),
			lsdir("a/d", m{"e": "FILE"}),
			lsdir("a/d.conflict.bob.unknown.0001-01-01_(1)", m{"c": "FILE", "f": "FILE"}),
			read("a/d/e", "world"),
		),
		as(alice,
			lsdir("a/", m{"d$": "DIR", "d.conflict.bob.unknown.0001-01-01$": "DIR", "d.conflict.bob.unknown.0001-01-01_\\(1\\)": "DIR"}),
...
func TestCrConflictWriteFileDouble(t *testing.T) {
	test(t,
		...
		as(bob, noSync(),
			...
			reenableUpdates(),
			lsdir("a/", m{"b$": "FILE", "b.conflict.bob.unknown.0001-01-01$": "FILE", "b.conflict.bob.unknown.0001-01-01_\\(1\\)": "FILE"}),
			read("a/b", "another write"),
			read("a/b.conflict.bob.unknown.0001-01-01", "uh oh"),
			read("a/b.conflict.bob.unknown.0001-01-01_(1)", "uh oh again!"),
		),
		as(alice,
			lsdir("a/", m{"b$": "FILE", "b.conflict.bob.unknown.0001-01-01$": "FILE", "b.conflict.bob.unknown.0001-01-01_\\(1\\)": "FILE"}),
			read("a/b", "another write"),
...
func TestCrRenameCycleWithTwoConflicts(t *testing.T) {
	test(t,
		...
		as(bob, noSync(),
			...
			reenableUpdates(),
			lsdir("a/", m{"b$": "DIR", "b.conflict.bob.unknown.0001-01-01": "FILE"}),
			read("a/b.conflict.bob.unknown.0001-01-01", "double uh oh"),
			lsdir("a/b/", m{"c": "DIR"}),
			lsdir("a/b/c", m{"b$": "SYM", "b.conflict.alice.unknown.0001-01-01": "FILE"}),
			lsdir("a/b/c/b", m{"c": "DIR"}),
		),
		as(alice,
			lsdir("a/", m{"b$": "DIR", "b.conflict.bob.unknown.0001-01-01": "FILE"}),
...
func TestCrRenameCycleWithConflictAndMergedDir(t *testing.T) {
	test(t,
		...
		as(bob, noSync(),
			...
			read("a/b.conflict.bob.unknown.0001-01-01", "uh oh"),
			lsdir("a/b/", m{"c": "DIR"}),
			lsdir("a/b/c", m{"b$": "DIR", "b.conflict.bob.unknown.0001-01-01": "SYM"}),
			lsdir("a/b/c/b.conflict.bob.unknown.0001-01-01", m{"c": "DIR"}),
			lsdir("a/b/c/b", m{}),
		),
		as(alice,
			lsdir("a/", m{"b$": "DIR", "b.conflict.bob.unknown.0001-01-01": "FILE"}),
		...
		as(bob,
			read("a/b/c/b.conflict.bob.unknown.0001-01-01/d", "hello"),
		),
	)
}
