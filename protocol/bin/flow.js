...
function analyzeMessages (json) {
  return Object.keys(json.messages).map(m => {
    ...
    const response = `export type ${capitalize(name)}Result = ${responseType === 'Null' ? 'void' : responseType}`
    ...
    */`
    ...
    const rpc = `export type ${capitalize(name)}Rpc = {
