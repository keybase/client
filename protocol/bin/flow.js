...
Object.keys(enabledCalls).forEach(rpc =>
    Object.keys(enabledCalls[rpc]).forEach(type => {
        if (!['engineSaga', 'promise', 'incoming', 'engineListener'].includes(type)) {
    ...
    return Object.keys(json.messages).reduce((map, m) => {
        ...
        const arr = message.request
            ...
            .map(r => {
                ...
                return `readonly ${r.name}${r.hasOwnProperty('default') || rtype.endsWith('| null') ? '?' : ''
                    }: ${rtype}`
            })
        ...
        if (engineSaga.length) {
            project.hasEngineSaga = true
        }
        ...
        if (rpcPromiseType || engineSagaType || engineListenerType || isUIMethod) {
            map[methodName] = {
                inParam,
...
function writeFlow(typeDefs, project) {
    ...
    const engineImports = [
        ...
        project.hasEngineSaga ? 'getEngineSaga' : '',
    ...
    const sagaImports = [
        project.hasEngineSaga || project.hasEngine ? 'type Effect' : '',
        project.hasEngineSaga ? 'call' : '',
    ]
        .filter(f => f.length)
        .join(', ')
    const sagaImport = sagaImports.length ? `import {${sagaImports}} from 'redux-saga/effects'` : ''
...
${messageTypesData}
    ...
    const data = [
        ...
        ...[...messagePromise, ...messageEngineSaga, ...messageEngineListener].sort(),
    ]
        .filter(Boolean)
        .join('\n')
...
`
    ...
    const messageEngineSaga = Object.keys(typeDefs.messages).map(k => typeDefs.messages[k].engineSaga)
    const messageEngineListener = Object.keys(typeDefs.messages).map(k => typeDefs.messages[k].engineListener)
    const data = [...consts, ...messagePromise, ...messageEngineSaga, ...messageEngineListener]
        .filter(Boolean)
        .sort()
        .join('\n')
