  ...
  return Object.keys(json.messages).map(m => {
    ...
    if (!isNotify) {
      ...
      if (type) {
        ...
        rt = `,responseError: RPCError, responseResult: ${capitalize(name)}Result`
      } else {
        r = ',response: CommonResponseHandler'
        rt = ',responseError: RPCError, responseResult: void'
      }
    } else {
      r = ''
      rt = ''
    ...
    if (isUIProtocol) {
      ...
      project.incomingTypes[`keybase.1.${json.protocol}.${m}`] = `{params: ${inParams ? `${inParams}` : 'void'}${rt}}`
    }
...
`
  ...
  const incomingType =
    ...
    Object.keys(project.incomingTypes).map(im => `  '${im}': ${project.incomingTypes[im]}`).join(',\n') +
    '|}\n'
  const toWrite = [typePrelude, typeDefs.join('\n'), incomingMap, incomingType].join('\n')
