#!/bin/bash
# Compares baseline vs current iOS screenshots using ImageMagick.
# Usage: ./visual-diff-compare-ios.sh
#
# Same methodology as visual-diff-compare.sh but for iOS simulator screenshots.
#
# Prerequisites:
#   - ImageMagick installed (brew install imagemagick)
#   - Baseline screenshots in /tmp/visual-diff-ios/baseline/
#   - Current screenshots in /tmp/visual-diff-ios/current/

set -euo pipefail

BASE_DIR="/tmp/visual-diff-ios"
...
if [[ ! -d "$BASELINE" ]]; then
  ...
  echo "Run: ./visual-diff-take-ios.sh baseline"
  exit 1
fi

if [[ ! -d "$CURRENT" ]]; then
  echo "ERROR: No current screenshots found at $CURRENT/"
  echo "Run: ./visual-diff-take-ios.sh current"
  exit 1
fi
...
echo "Comparing baseline vs current (iOS)..."
echo ""
printf "%-12s %10s  %s\n" "Tab" "Diff px" "Status"
printf "%-12s %10s  %s\n" "---" "-------" "------"
...
for img in "$BASELINE"/*.png; do
  ...
  # Compare (exit code 1 = images differ, not an error)
  ...
  if [[ "$diff_px" -eq 0 ]]; then
    status="PERFECT"
  elif [[ "$diff_px" -lt 500 ]]; then
    status="ok (subpixel)"
  elif [[ "$diff_px" -lt 5000 ]]; then
    status="REVIEW"
...
if [[ ${#ISSUES[@]} -gt 0 ]]; then
  ...
  echo "  - Subpixel noise: scattered faint red dots, typically <500px on iOS — safe to ignore"
