...
function loadBaseline(dir) {
  ...
  if (fs.existsSync(desktopFile)) {
    ...
    return {fps: data.fps, longTasks: data.longTasks ?? null, memory: data.memory ?? null, react: data.react ?? null, type: 'desktop'}
  }
  ...
  if (fs.existsSync(iosFpsFile)) {
    ...
    return {fps, longTasks: null, memory: null, react, type: 'ios'}
  }
  return null
}
