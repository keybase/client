...
// SecurityPrefsPromptingHoc prompts user about going to security preferences
// to allow the kext if needed. It prompts at most once per app-restart. Which
// means for example, if the user goes to Files tab and gets an prompt. they
// won't get prompted again in Settings, or after they click "Back" in the
// prompt and go back to Files tab again. This is to avoid spamming the user.
// We have a link in the Settings page so if the user wants they can still find
// the instructions.
const SecurityPrefsPromptingHoc = isMobile
  ...
  : compose(
      ...
      branch(({shouldPromptSecurityPrefs, showSecurityPrefsOnce}) => {
        shouldPromptSecurityPrefs && showSecurityPrefsOnce()
        return shouldPromptSecurityPrefs
      }, renderNothing)
    )

export {SecurityPrefsPromptingHoc}
