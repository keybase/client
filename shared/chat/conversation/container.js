...
const mapStateToProps = (
  ...
  if (selectedConversationIDKey !== Constants.nothingSelected) {
    ...
    finalizeInfo = state.chat
      .get('finalizedState')
      .get(selectedConversationIDKey)
    supersedes = Constants.convSupersedesInfo(
      selectedConversationIDKey,
      state.chat
    )
    supersededBy = Constants.convSupersededByInfo(
      selectedConversationIDKey,
      state.chat
    )
    ...
    if (conversationState) {
      ...
      showLoader =
        !(selected && selected.state === 'unboxed') ||
...
const mapDispatchToProps = (
  ...
  _onAttach: (
    ...
    dispatch(
      navigateAppend([
        {
          props: {conversationIDKey: selectedConversation, inputs},
          selected: 'attachmentInput',
        },
      ])
    )
  },
...
export default compose(
  ...
  branch(
    (props: Props) =>
      props.selectedConversationIDKey === Constants.nothingSelected,
    renderComponent(NoConversation)
  ),
  branch(
    (props: Props) => !props.finalizeInfo && props.rekeyInfo,
    renderComponent(Rekey)
  ),
  ...
  withHandlers({
    ...
    onEditLastMessage: props => () =>
      props.setEditLastMessageCounter(props.editLastMessageCounter + 1),
    onFocusInput: props => () =>
      props.setFocusInputCounter(props.focusInputCounter + 1),
    onScrollDown: props => () =>
      props.setListScrollDownCounter(props.listScrollDownCounter + 1),
