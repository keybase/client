...
const mapStateToProps = (state: TypedState) => {
  ...
  const finalizeInfo = state.chat
    .get('finalizedState')
    .get(selectedConversationIDKey)
  const _supersededBy = Constants.convSupersededByInfo(
  ...
  const selected =
    inbox &&
    inbox.find(
      inbox => inbox.get('conversationIDKey') === selectedConversationIDKey
    )
...
const mapDispatchToProps = (dispatch: Dispatch) => ({
  ...
  startConversation: (users: Array<string>) =>
    dispatch(Creators.startConversation(users, true)),
})
  ...
  onOpenNewerConversation: stateProps._supersededBy
    ? () => {
        stateProps._supersededBy &&
          ...
          dispatchProps.onOpenConversation(
            stateProps._supersededBy.conversationIDKey
          )
      }
