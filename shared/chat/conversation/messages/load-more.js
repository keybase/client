...
const MessageLoadingMore = ({type}: Props) => (
  <Box>
    <Box style={secureStyle}>
      {type === 'noMoreToLoad' && <Icon type={isMobile ? 'icon-secure-static-266' : 'icon-secure-266'} />}
    </Box>
    {/*
    showTeamOffer &&
    <Box style={moreStyle}><CreateTeamNotice /></Box> */}
    <Box style={type === 'moreToLoad' ? moreStyle : noneStyle}>
