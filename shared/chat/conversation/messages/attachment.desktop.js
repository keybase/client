...
class _AttachmentMessage extends PureComponent<void, Props & {onAttachmentLoaded: (event: any) => void, onIconClick: (event: any) => void, onOpenInPopup: (event: any) => void}, void> {
  render () {
    const {message, style, includeHeader, isFirstNewMessage, onAttachmentLoaded, onLoadAttachment, onOpenInFileUI, onOpenInPopup, onIconClick} = this.props
          ...
          <Box style={{...globalStyles.flexBoxRow, flex: 1, paddingTop: (includeHeader ? globalMargins.tiny : 0)}}>
            ...
            <Box style={{...globalStyles.flexBoxColumn, flex: 1}}>
              ...
              <Box style={{...globalStyles.flexBoxRow, flex: 1}}>
                ...
                {!!message.previewPath && message.previewType === 'Image' && <Box style={{marginTop: globalMargins.xtiny, flex: 1}} onClick={onOpenInPopup}><img src={message.previewPath} onLoad={onAttachmentLoaded} /></Box>}
...
export default withHandlers({
  onAttachmentLoaded: (props: Props) => event => {
    props.onAttachmentLoaded(props.message.messageID, event)
  },
