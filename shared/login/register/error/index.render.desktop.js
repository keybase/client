...
const renderError = (error: RPCError) => {
  ...
  switch (error.code) {
    case ConstantsStatusCode.scdevicenoprovision:
      return (
        <Box style={styleContent}>
          <p style={styleText}><Text type='Body'>You can't authorize by passphrase, since you have established device or paper keys. You can go back and pick a device or paper key, or <br/><Text type='BodyPrimaryLink' onClick={() => openURL('https://keybase.io/#account-reset')}>reset your account entirely</Text>.</Text></p>
        </Box>)
      ...
      if (fields.has_active_device) {
        return (
          <Box style={styleContent}>
            ...
            <p style={{...styleText, textAlign: 'left', marginTop: 16}}><Text type='BodySemibold'>You have options:</Text></p>
            <p style={styleList}>
              <Text type='Body'> - Go back and select a device or paper key</Text><br/>
              <Text type='Body'> - Install Keybase on a machine that has your PGP private key in it</Text><br/>
              <Text type='Body'> - Login to the website and host an encrypted copy of your PGP private key</Text><br/>
              <Text type='Body'> - or, <Text type='BodyPrimaryLink' onClick={() => openURL('https://keybase.io/#account-reset')}>reset your account entirely</Text>.</Text>
            </p>
          </Box>)
      } else {
        return (
          <Box style={styleContent}>
            ...
            <p style={styleList}>
              ...
              <Text type='Body'> - Install GPG on this machine and import your PGP private key into it</Text><br/>
              <Text type='Body'> - Install Keybase on a different machine that has your PGP key</Text><br/>
              <Text type='Body'> - Login to the website and host an encrypted copy of your PGP private key</Text><br/>
              <Text type='Body'> - Or, <Text type='BodyPrimaryLink' onClick={() => openURL('https://keybase.io/#account-reset')}>reset your account entirely</Text>.</Text>
            </p>
          </Box>)
      ...
      return (
        <Box style={styleContent}>
          {error.desc ? <Markdown>{error.desc}</Markdown> : (
            <p style={styleText}><Text type='Body'>Your PGP keychain has multiple keys installed, and we're not sure which one to use to provision your account. Please run <Text type='TerminalInline'>keybase login</Text> on the command line to continue.</Text></p>
          )}
        </Box>
      )
    case ConstantsStatusCode.scnotfound:
      return (
        <Box style={styleContent}>
          <p style={styleText}><Text type='Body'>The username you provided doesn't exist on Keybase, please try logging in again with a different username.</Text></p>
        </Box>
      )
    case ConstantsStatusCode.scbadloginpassword:
      return (
        <Box style={styleContent}>
          <p style={styleText}>
            <Text type='Body'>The server rejected your login attempt.</Text><br/>
            <Text type='BodyPrimaryLink' onClick={() => openURL('https://keybase.io/#password-reset')}>Reset your passphrase</Text>
          </p>
        </Box>)
    ...
    case ConstantsStatusCode.sckeynosecret:
      return (
        <Box style={styleContent}>
          ...
          <p style={styleList}>
            <Text type='Body'> - Use <Text type='TerminalInline'>keybase login</Text> on the command line to log in</Text><br/>
            <Text type='Body'> - Install GPG, put your PGP private key on this machine and try again</Text><br/>
    ...
    default:
      return <Box style={styleContent}><Text type='Body'>Unknown error: {error.message}</Text></Box>
  }
}
