  ...
  switch (error.code) {
    case ConstantsStatusCode.scdevicenoprovision:
      return (
        <Box style={styleContent}>
          <Box style={styleText}><Text type='Body'>You can't authorize by passphrase, since you have established device or paper keys. You can go back and pick a device or paper key, or <Text type='BodyPrimaryLink' onClick={() => openURL('https://keybase.io/#account-reset')}>reset your account entirely</Text>.</Text></Box>
        </Box>)
    ...
    case ConstantsStatusCode.sckeynomatchinggpg:
      if (fields.has_active_device) {
        return (
          <Box style={styleContent}>
            <Box style={styleText}><Text type='Body'>You can't provision using solely a passphrase, since you have active device keys.</Text></Box>
            <Box style={{...styleText, textAlign: 'left', marginTop: 16}}><Text type='BodySemibold'>You have options:</Text></Box>
            <Box style={{styleList}}>
        ...
        return (
          <Box style={styleContent}>
            <Box style={styleText}><Text type='Body'>You can't provision using a passphrase, since you've established a PGP key.</Text></Box>
            <Box style={{...styleText, textAlign: 'left', marginTop: 16}}><Text type='BodySemibold'>You have options:</Text></Box>
            <Box style={styleList}>
              <Text type='Body'> - Use <Text type='TerminalInline'>keybase login</Text> on the command line to log in</Text>
      ...
      return (
        <Box style={styleContent}>
          {error.desc ? <Markdown>{error.desc}</Markdown> : (
            <Box style={styleText}><Text type='Body'>Your PGP keychain has multiple keys installed, and we're not sure which one to use to provision your account. Please run <Text type='TerminalInline'>keybase login</Text> on the command line to continue.</Text></Box>
          )}
        </Box>
      )
    case ConstantsStatusCode.scnotfound:
      return (
        <Box style={styleContent}>
          <Box style={styleText}><Text type='Body'>The username you provided doesn't exist on Keybase, please try logging in again with a different username.</Text></Box>
        </Box>
      )
      ...
      return (
        <Box style={styleContent}>
          ...
          <Box style={styleList}>
            ...
            <Text type='Body'> - Or, <Text type='BodyPrimaryLink' onClick={() => openURL('https://keybase.io/#account-reset')}>reset your account and start fresh</Text></Text>
          </Box>
        </Box>)
    default:
