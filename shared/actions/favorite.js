...
function favoriteSwitchTab(showingPrivate: boolean): FavoriteSwitchTab {
  return {
    type: Constants.favoriteSwitchTab,
    payload: {showingPrivate},
    error: false,
  }
}
...
function toggleShowIgnored(isPrivate: boolean): FavoriteToggleIgnored {
  return {
    type: Constants.favoriteToggleIgnored,
    payload: {isPrivate},
    error: false,
  }
}
...
function* _listSaga(): SagaGenerator<any, any> {
  ...
  try {
    ...
    const listedAction: FavoriteListed = {
      type: Constants.favoriteListed,
      payload: {folders: state},
    }
    ...
    const debouncedKBFSStopped = _.debounce(() => {
      if (_kbfsUploadingState === true) {
        ...
        dispatch({
          ...
          payload: {isAsyncWriteHappening: false},
        })
      }
    }, 2000)
