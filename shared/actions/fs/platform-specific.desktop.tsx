...
import {
  makeRetriableErrorHandler,
  makeUnretriableErrorHandler
} from "./shared";
...
const _openPathInSystemFileManagerPromise = (
  openPath: string,
  isFolder: boolean
): Promise<void> =>
...
const openLocalPathInSystemFileManager = async (
  ...
  try {
    ...
    return _openPathInSystemFileManagerPromise(
...
const _rebaseKbfsPathToMountLocation = (
  kbfsPath: Types.Path,
  ...
  state.fs.sfmi.directMountDir
    ? _openPathInSystemFileManagerPromise(
        ...
        ![Types.PathKind.InGroupTlf, Types.PathKind.InTeamTlf].includes(
          ...
          Constants.getPathItem(state.fs.pathItems, action.payload.path)
...
const refreshDriverStatus = async (
  ...
  action:
    ...
    | FsGen.RefreshDriverStatusPayload
) => {
  if (
    ...
    if (
      isWindows &&
      status.installStatus !== RPCTypes.InstallStatus.installed
    ) {
...
const fuseInstallResultIsKextPermissionError = (
  result: RPCTypes.InstallResult
): boolean =>
  !!result &&
  ...
  result.componentResults.findIndex(
    c =>
      c.name === "fuse" &&
      c.exitCode === Constants.ExitCodeFuseKextPermissionError
  ) !== -1;
  ...
  if (fuseInstallResultIsKextPermissionError(result)) {
    return [
      ...
      ...(action.payload.isRetry
        ? []
        : [RouteTreeGen.createNavigateAppend({ path: ["kextPermission"] })])
    ];
  } else {
...
const uninstallKBFSConfirm = async () => {
  const action = await new Promise<TypedActions | false>(resolve =>
    SafeElectron.getDialog()
      ...
      .then(({ response }) =>
        response === 0 ? resolve(FsGen.createDriverDisabling()) : resolve(false)
      )
  );
  return action;
};
...
const uninstallDokan = (state: TypedState) => {
  ...
  const execPath: string =
    state.fs.sfmi.driverStatus.dokanUninstallExecPath || "";
  ...
  SafeElectron.getDialog()
    .showOpenDialog(SafeElectron.getCurrentWindowFromRemote(), {
      properties: [
        ...
        ...(["file", "both"].includes(action.payload.type)
          ? (["openFile"] as const)
          : []),
        ...(["directory", "both"].includes(action.payload.type)
...
const openAndUpload = async (action: FsGen.OpenAndUploadPayload) => {
  ...
  return localPaths.map(localPath =>
    FsGen.createUpload({ localPath, parentPath: action.payload.parentPath })
  );
};
...
const changedFocus = (
  state: TypedState,
  action: ConfigGen.ChangedFocusPayload
) =>
...
const refreshMountDirs = async (
  ...
  return [
    ...
    ...(action.type !== FsGen.refreshMountDirsAfter10s
      ? [FsGen.createRefreshMountDirsAfter10s()]
      : [])
  ];
};
...
function* platformSpecificSaga() {
  ...
  yield* Saga.chainAction2(
    [
      ...
      FsGen.setDriverStatus,
      FsGen.refreshMountDirsAfter10s
    ],
    refreshMountDirs
  );
  ...
  if (isWindows) {
    ...
    yield* Saga.chainAction2(
      FsGen.driverDisable as any,
      uninstallDokanConfirm as any
    );
