...
function setTextCode(
  ...
  return {
    ...
    payload: {
      enterCodeErrorText: previousErr,
...
function setQRCode(code: string): Constants.SetQRCode {
  return {
    ...
    payload: {qrCode: new HiddenString(qrGenerate(code))},
  }
}
...
): Constants.SubmitPassphrase {
  if (typeof passphrase === 'string') {
    throw new Error('DEV: passphrase is a string, should be a hidden string!')
  }
  return {
    type: Constants.submitPassphrase,
    payload: {passphrase, storeSecret},
  }
}
...
function setOtherDeviceCodeState(
  otherDeviceRole: Constants.DeviceRole
): Constants.SetOtherDeviceCodeState {
...
function waitingForResponse(
  waiting: boolean
): TypedAction<'login:waitingForResponse', boolean, void> {
