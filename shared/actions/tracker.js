    ...
    new Promise((resolve, reject) => {
      ...
      RPCTypes.identifyIdentify2Rpc({
        ...
        callback: (error, response) => {
          ...
          dispatch({
            type: Constants.identifyFinished,
            payload: {username: uid || userAssertion},
          })
          resolve()
        },
      })
  ...
  return (dispatch, getState) => {
    ...
    const dispatchErrorAction = errText => {
      ...
      dispatch({
        ...
        payload: {username, extraText: errText},
      })
    }
...
function _serverCallMap(
  ...
  return {
    ...
    'keybase.1.identifyUi.displayKey': ({key}, response) => {
      ...
      requestIdle(() => {
        if (key.breaksTracking) {
          dispatch({
            type: Constants.updateEldestKidChanged,
            payload: {username},
          })
          dispatch({
            type: Constants.updateReason,
            payload: {
              username,
              reason: `${username} has reset their account!`,
            },
          })
          dispatch({
            type: Constants.updateProofState,
            payload: {username},
          })
          if (!isGetProfile) {
            dispatch({
              type: Constants.showTracker,
              payload: {username},
            })
          }
      ...
      requestIdle(() => {
        if (isGetProfile) {
          ...
          dispatch({
            ...
            payload: {
              uid: card.uid,
              goodTill: Date.now() + cachedIdentifyGoodUntil,
            },
          })
        }
    ...
    'keybase.1.identifyUi.reportTrackToken': ({trackToken}, response) => {
      ...
      requestIdle(() => {
        dispatch({
          type: Constants.updateTrackToken,
          payload: {username, trackToken},
        })
