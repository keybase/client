...
function* unboxConversations(action: ChatGen.UnboxConversationsPayload): SagaGenerator<any, any> {
  ...
  logger.info(`unboxConversations: unboxing ${conversationIDKeys.length} convs, because: ${reason}`)
  ...
  try {
    ...
    if (error instanceof RPCTimeoutError) {
      logger.warn('timed out request for unboxConversations, bailing')
      yield Saga.put.resolve(
        ChatGen.createReplaceEntity({
          keyPath: ['inboxUntrustedState'],
      ...
      logger.warn('Error in loadInboxRpc')
      logger.debug('Error in loadInboxRpc', error)
    }
  }
  if (forInboxSync) {
