...
if [ "$arg" = "ios" ]; then
	...
	set +e
	OUTPUT="$(gomobile bind -target=android -tags="android $tags" -ldflags "$android_ldflags" -o "$android_dest" "$package" 2>&1)"
	set -e
	if [[ $OUTPUT == *gomobile* ]]; then
		build_gomobile
		gomobile bind -target=android -tags="android $tags" -ldflags "$android_ldflags" -o "$android_dest" "$package"
	else
		echo $OUTPUT
	fi
else
