...
how about ~_*bold and italic and strike through?*_~ - now - _*some bold* and just italic_ bold.*with*.punctuation!`}
    ...
    .add('special chars in code block/S', () => (
      <Markdown simple={true}>{`I think we should try to use \`if else\` statements \`\`\`if (var == "foo")
  echo "foo";
else echo "bar";
  // this should be *asterisk* \`\`\``}</Markdown>
    ...
    .add('Escaped chars/S', () => <Markdown simple={true}>{'\\*foo\\* I should see asterisks'}</Markdown>)
    .add('links/S', () => (
      <Markdown simple={true}>{`
      Ignore:
        a...b,
...
`}</Markdown>
    ...
    .add('Quotes/S', () => (
      <Markdown simple={true}>{`> this is quoted
