...
function isStyleInViewport(style, popupCoords: DOMRect): boolean {
  const {
    pageYOffset,
    pageXOffset,
  }: {
...
function pushStyleIntoViewport(style, popupCoords: DOMRect) {
  const {
    pageYOffset,
    pageXOffset,
  }: {
...
function ModalPositionRelative<PP>(
  ...
  > {
    ...
    _computeStyle = (targetRect: DOMRect | null) => {
    ...
    getSnapshotBeforeUpdate(prevProps) {
      const {width, height} = this.popupNode ? this.popupNode.getBoundingDOMRect() : {height: -1, width: -1}
      return {height, width}
    }
