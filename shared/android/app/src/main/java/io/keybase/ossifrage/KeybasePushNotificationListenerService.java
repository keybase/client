            ...
            WithBackgroundActive withBackgroundActive =
                () -> {
                  try {
                    ...
                    NativeLogger.error(
                        "Go Couldn't handle background notification: " + ex.getMessage());
                  }
                };
...
interface WithBackgroundActive {
  ...
  default void whileActive(Context context) throws Exception {
    ...
    if (Keybase.isAppStateForeground()) {
      ...
      // Check if we are foreground now for some reason. In that case we don't want to go background
      // again
      if (Keybase.isAppStateForeground()) {
        return;
      }
