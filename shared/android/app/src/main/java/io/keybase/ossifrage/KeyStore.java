...
public class KeyStore implements UnsafeExternalKeyStore {
  ...
  public synchronized byte[] getUsersWithStoredSecretsMsgPack(final String serviceName)
    ...
    try {
      ...
      NativeLogger.info(
          "KeyStore: got " + userNames.size() + " users with stored secrets for " + serviceName);
  ...
  public synchronized byte[] retrieveSecret(final String serviceName, final String key)
    ...
    try {
      ...
      try {
        ...
        // Invalid key, this can happen when a user changes their lock screen from something to
        // nothing
        // or enrolls a new finger. See
        // https://developer.android.com/reference/android/security/keystore/KeyPermanentlyInvalidatedException.html
