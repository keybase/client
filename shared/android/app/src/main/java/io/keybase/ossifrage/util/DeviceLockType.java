...
/**
 * Code to check if the device has some lockscreen setup. Adapted from:
...
public class DeviceLockType {
  ...
  /**
   * This constant means that android using some unlock method not described here. Possible new
   * methods would be added in the future releases.
   */
  public static final int SOMETHING_ELSE = 0;

  /**
   * Android using "None" or "Slide" unlock method. It seems there is no way to determine which
  ...
  /**
   * Android using "Face Unlock" with "Pattern" as additional unlock method. Android don't allow you
   * to select "Face Unlock" without additional unlock method.
   */
  public static final int FACE_WITH_PATTERN = 3;

  /**
   * Android using "Face Unlock" with "PIN" as additional unlock method. Android don't allow you to
   * select "Face Unlock" without additional unlock method.
   */
  public static final int FACE_WITH_PIN = 4;

  /**
   * Android using "Face Unlock" with some additional unlock method not described here. Possible new
   * methods would be added in the future releases. Values from 5 to 8 reserved for this situation.
   */
  ...
  public static int getCurrent(ContentResolver contentResolver) {
    long mode =
        android.provider.Settings.Secure.getLong(
            contentResolver, PASSWORD_TYPE_KEY, DevicePolicyManager.PASSWORD_QUALITY_SOMETHING);
    if (mode == DevicePolicyManager.PASSWORD_QUALITY_SOMETHING) {
      if (android.provider.Settings.Secure.getInt(
              contentResolver, Settings.Secure.LOCK_PATTERN_ENABLED, 0)
          == 1) {
        return PATTERN;
