...
export default Container.remoteConnect(
  ...
  (stateProps, dispatchProps, _ownProps: {}) => {
    const {_showUser, _onRetrySync, ...dRest} = dispatchProps
    const {config, ...rest} = stateProps
    const {username} = config
    return {
      ...rest,
      ...config,
      ...dRest,
      onRetrySync: () => dispatchProps._onRetrySync(username),
      showUser: () => dispatchProps._showUser(username),
    }
  }
)(Menubar)
