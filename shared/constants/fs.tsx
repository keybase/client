...
export const makeSortSetting: I.Record.Factory<Types._SortSetting> = I.Record({
  sortBy: 'name',
  sortOrder: 'asc',
} as Types._SortSetting)
...
export type FavoritesListResult = {
  users: {[K in string]: string}
  devices: {[K in string]: Types.Device}
...
export const createFavoritesLoadedFromJSONResults = (
  ...
  username: string,
  loggedIn: boolean
): FsGen.FavoritesLoadedPayload | null => {
  const favoritesResult = ((txt: string): FavoritesListResult | null => {
    try {
  ...
  const tlfs: {
    private: {[K in string]: Types.Tlf}
    public: {[K in string]: Types.Tlf}
    team: {[K in string]: Types.Tlf}
  } = folders.reduce(
    (tlfs, folder) => {
      const {
        name,
...
export const computeBadgeNumberForAll = (tlfs: Types.Tlfs): number =>
  ['private', 'public', 'team']
    .map(tlfType => computeBadgeNumberForTlfList(getTlfListFromType(tlfs, tlfType as Types.TlfType)))
...
export const parsedPathTeamList: Types.ParsedPathTlfList = I.Record({
  kind: 'tlf-list',
  tlfType: 'team',
