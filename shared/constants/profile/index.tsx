  ...
  const submitCryptoAddress = (wantedFamily: 'bitcoin' | 'zcash') => {
    ...
    const f = async () => {
      ...
      try {
        ...
        routerState.dispatch.navigateAppend('profileConfirmOrPending')
      } catch (_error) {
  ...
  const dispatch: State['dispatch'] = {
    addProof: (platform, reason) => {
      ...
      const f = async () => {
        ...
        switch (service) {
          case 'dnsOrGenericWebSite':
            storeRegistry.getState('router').then(routerState => {
              routerState.dispatch.navigateAppend('profileProveWebsiteChoice')
            })
            return
          case 'zcash': //  fallthrough
          case 'btc':
            storeRegistry.getState('router').then(routerState => {
              routerState.dispatch.navigateAppend('profileProveEnterUsername')
            })
            return
          case 'pgp':
            storeRegistry.getState('router').then(routerState => {
              routerState.dispatch.navigateAppend('profilePgp')
            })
            return
          default:
        }
        ...
        const loadAfter = async () => {
          ...
          const trackerState = await storeRegistry.getState('tracker2')
          trackerState.dispatch.load({
            ...
            customResponseIncomingCallMap: {
              ...
              'keybase.1.proveUi.outputInstructions': ({proof}, response) => {
                ...
                if (service) {
                  ...
                  storeRegistry.getState('router').then(routerState => {
                    routerState.dispatch.navigateAppend('profilePostProof')
                  })
                } else if (proof) {
                  set(s => {
              ...
              'keybase.1.proveUi.promptUsername': (args, response) => {
                ...
                if (service) {
                  ...
                  storeRegistry.getState('router').then(routerState => {
                    routerState.dispatch.navigateAppend('profileGenericEnterUsername')
                  })
                }
              },
            },
    ...
    backToProfile: () => {
      ...
      setTimeout(() => {
        storeRegistry.getState('current-user').then(currentUserState => {
          get().dispatch.showUserProfile(currentUserState.username)
        })
      }, 100)
    },
    checkProof: () => {
    ...
    finishRevoking: () => {
      storeRegistry.getState('current-user').then(currentUserState => {
        ...
        storeRegistry.getState('tracker2').then(trackerState => {
          trackerState.dispatch.load({
            assertion: username,
            guiID: generateGUIID(),
            inTracker: false,
    ...
    generatePgp: () => {
      const f = async () => {
        ...
        storeRegistry.getState('router').then(routerState => {
          routerState.dispatch.navigateAppend('profileGenerate')
        })
          ...
          await T.RPCGen.pgpPgpKeyGenDefaultRpcListener({
            customResponseIncomingCallMap: {
              ...
              'keybase.1.pgpUi.shouldPushPrivate': ({prompt}, response) => {
                storeRegistry.getState('router').then(routerState => {
                  routerState.dispatch.navigateAppend('profileFinished')
                })
                set(s => {
    ...
    submitRevokeProof: proofId => {
      const f = async () => {
        const currentUserState = await storeRegistry.getState('current-user')
        const trackerState = await storeRegistry.getState('tracker2')
        const you = trackerState.getDetails(currentUserState.username)
    ...
    submitUnblockUser: (username, guiID) => {
      const f = async () => {
        try {
          ...
          const trackerState = await storeRegistry.getState('tracker2')
          trackerState.dispatch.updateResult(guiID, 'error', `Failed to unblock ${username}: ${error.desc}`)
        }
      }
      ignorePromise(f())
    },
