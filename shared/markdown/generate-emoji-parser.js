...
function genEmojiData() {
  ...
  emojiData.forEach(emoji => {
    if (emoji.skin_variations) {
      Object.keys(emoji.skin_variations).forEach((v, idx) =>
        addEmojiLiteral(v, emoji.short_name, idx + 1)
      )
    }
...
function buildParser() {
  ...
  const parserJS = peg.generate(generatedSource, {
    output: 'source',
    format: 'commonjs',
  })
  const fullParserJS = prependJS + parserJS + appendJS

  const parserPath = path.format({
