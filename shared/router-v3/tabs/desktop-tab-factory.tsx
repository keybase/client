...
import AccountSwitcher from '../../people/account-switcher/container'
...
const data = {
  blankTab: {icon: 'iconfont-nav-2-chat', label: 'Blank'},
  ...
  return username ? (
    <NavigationHelpersContext.Provider value={navigation}>
      <Kb.Box2 direction="horizontal" fullWidth={true} fullHeight={true}>
        <Kb.Box2 className="tab-container" direction="vertical" fullHeight={true}>
          ...
          {state.routes.map((route, i) => (
            <Tab
              ...
              tab={route.name}
              index={i}
              isSelected={selectedTab === route.name}
              onTabClick={() => {
              ...
              badge={
                route.name === Tabs.fsTab && fsCriticalUpdate
                  ? (badgeNumbers.get(route.name) ?? 0) + 1
                  : badgeNumbers.get(route.name)
              }
            />
          ))}
          <RuntimeStats />
        ...
        <Kb.BoxGrow>{descriptors[state.routes[state.index].key].render()}</Kb.BoxGrow>
      </Kb.Box2>
...
export default createNavigatorFactory(TabNavigator)() as any
