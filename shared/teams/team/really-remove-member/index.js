...
const ReallyRemoveMember = (props: Props) => {
  const member = props.username || props.email
  return (
    <MaybePopup onClose={props.onClose}>
      <Box
        style={{...globalStyles.flexBoxColumn, alignItems: 'center', flex: 1, padding: globalMargins.large}}
      >
        <Avatar username={member} size={64} />
        ...
        <Text type="Header" style={{marginBottom: globalMargins.large, marginTop: globalMargins.large}}>
          Are you sure you want to remove {member} from {props.name}?
        </Text>
        <Text type="Body" style={{maxWidth: 450, textAlign: 'center'}}>
          {member} will lose access to all the
          {' '}
          {props.name}
          {' '}
        ...
        >
          ...
          />
          <Button type="Danger" onClick={props.onRemove} label={`Yes, remove ${member}`} />
        </Box>
      </Box>
    </MaybePopup>
  )
