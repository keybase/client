...
function diffAndStatusMeta(
  ...
  diffMeta: ?Constants.SimpleProofMeta,
  statusMeta: ?Constants.SimpleProofMeta,
} {
...
function remoteProofToProof(
  ...
  const proofState: Constants.SimpleProofState =
    (lcr &&
      proofStateToSimpleProofState(
        lcr.proofResult.state,
        lcr.diff,
        lcr.remoteDiff,
        lcr.breaksTracking
      )) ||
  ...
  return {
    ...
    profileUrl: rp.displayMarkup &&
      proofUrlToProfileUrl(rp.proofType, rp.displayMarkup, rp.key, humanUrl),
    state: proofState,
...
export function overviewStateOfProofs(
  proofs: Array<Constants.Proof>
): Constants.OverviewProofState {
