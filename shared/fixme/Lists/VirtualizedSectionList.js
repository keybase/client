...
export type Props<SectionT> = RequiredProps<SectionT> & OptionalProps<SectionT> & VirtualizedListProps
...
class VirtualizedSectionList<SectionT: SectionBase>
  ...
  _subExtractor(
    ...
    for (let ii = 0; ii < this.props.sections.length; ii++) {
      ...
      warning(key != null, 'VirtualizedSectionList: A `section` you supplied is missing the `key` property.')
...
class ItemWithSeparator extends React.Component {
  ...
  render() {
    ...
    const leadingSeparator =
      LeadingSeparatorComponent && <LeadingSeparatorComponent {...this.state.leadingSeparatorProps} />
