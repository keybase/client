...
type OwnProps = {
  ...
  onChangeService: (newService: ServiceIdWithContact) => null
  incHighlightIndex: (maxIndex: number) => null
  decHighlightIndex: () => null
  resetHighlightIndex: (resetToHidden?: boolean) => null
}
...
const deriveSearchResults = memoize(
  (searchResults: Array<User> | null, teamSoFar: I.Set, myUsername: string, followingState: I.Set) =>
    searchResults &&
...
const deriveTeamSoFar = memoize((teamSoFar: I.Set) =>
...
const deriveServiceResultCount: (
  ...
  [key: ServiceIdWithContact]: number | null
} = memoize((searchResults: SearchResults, query) =>
...
const deriveOnChangeText = memoize(
  (
    onChangeText: (newText: string) => null,
    search: (text: string, service: ServiceIdWithContact) => null,
    selectedService: ServiceIdWithContact,
...
const deriveOnDownArrowKeyDown = memoize(
  (maxIndex: number, incHighlightIndex: (maxIndex: number) => null) => () => incHighlightIndex(maxIndex)
)
