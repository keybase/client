  ...
  const makeRules = nodeThread => {
    ...
    const babelRule = {
      ...
      options: {
        ...
        presets: [
          ['@babel/preset-env', {debug: false, modules: false, targets: {electron: '8.0.1'}}],
    ...
    return [
      ...
      {
        exclude: /((node_modules\/(?!universalify|fs-extra|react-redux|redux-saga|react-gateway|react-native-svg))|\/dist\/)/,
        test: /\.(ts|js)x?$/,
        use: [babelRule],
      },
      {
