...
const RemoteContainer = (d: DeserializeProps) => {
  const {avatarRefreshCounter, darkMode, trackerUsername, tracker, followers, following, username} = d
  const {httpSrvToken, httpSrvAddress, infoMap, blockMap} = d
  const {usernameToDetails} = tracker
  ...
  React.useEffect(() => {
    const id = setTimeout(() => {
      replaceTracker(tracker.usernameToDetails)
    }, 1)
    return () => {
      clearTimeout(id)
    }
