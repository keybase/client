...
export function printTimingStats (shouldRun: boolean, statSink: StatSink, warnOnSlowPokes?: boolean, stdDevThreshold?: number) {
  if (statSink.totalTime > 0 && statSink.totalActions > 0 && shouldRun) {
    const mean = statSink.totalTime / statSink.totalActions
    const variance = (statSink.timings.map(t => Math.pow((t - mean), 2)).reduce((acc, t) => t + acc, 0)) / (statSink.totalActions - 1)
