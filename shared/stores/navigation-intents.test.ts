...
// The module remembers acked push tap ids for the life of the file, the same as it does for the JS
// runtime, so ids must not repeat across tests any more than they do across taps.
...
// Native holds the tap until its id is acked, and enqueuing is not acting on it: the ack is a
// distinct, explicit step, never implied by enqueuing.
...
// A peek never clears the held tap, so a repeated peek re-delivers the same id. Re-enqueuing it
// must not queue (and so navigate) a second time.
...
// A redelivery after the tap has already been consumed -- the native ack did not land, so native
// still holds it -- must not navigate a second time, however long ago that was, but the ack itself
// is repeated: nothing else will ever ask native to clear that tap again.
...
test('merging a newer tap into the same-URL pending intent adopts its id instead of acking the old one', () => {
  ...
  // Native replaces an unacked tap outright on a new tap, so by the time this lands the older
  // one is already gone there; acking it here would be a pointless extra call.
  ...
  expect(ack).toHaveBeenCalledTimes(1)
  expect(ack).toHaveBeenCalledWith(newer)
})
...
test('a tap enqueued again inside the duplicate window of its own navigation acks immediately', () => {
  ...
  expect(useNavigationIntentsState.getState().intent).toBeUndefined()
  expect(ack).toHaveBeenCalledTimes(1)
  expect(ack).toHaveBeenCalledWith(second)
})
...
test('a pending tap superseded by an unrelated enqueue acks the route it loses', () => {
  ...
  // A plain deep link (emitDeepLink) for an unrelated URL: native was never told this tap was
  // acted on, so without an explicit ack here the next peek would hand the same tap back.
