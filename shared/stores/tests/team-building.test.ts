...
test('finishedTeamBuilding clears transient state but preserves namespace and selections', () => {
  ...
  expect(store.getState()).not.toHaveProperty('searchLimit')
  expect(store.getState()).not.toHaveProperty('searchQuery')
  expect(store.getState()).not.toHaveProperty('selectedService')
})
