...
function updatedFonts () {
  ...
  project.iconSets.forEach(set => {
    set.icons.forEach(icon => {
      ...
      if (name.startsWith(fontPrefix)) {
        ...
        sizes[shortName] = icon.grid
        fontNames[`'${shortName}'`] = true
      } else {
  ...
  const fontSizes = JSON.stringify(sizes, null, 2).replace(/"/g, "'").replace('\n}', ',\n}\n')

  const flowFontIcons = Object.keys(fontNames).sort().map(f => `${f}: true`).join(',\n  ')

  const flowIcons = fs.readdirSync('../shared/images/icons')
    .filter(f => f.indexOf('@') === -1 && f.startsWith('icon-'))
    .map(f => `'${f.slice(0, -4)}': true`).join(',\n  ')
...
export const fontSizes: {[key: string]: number} = ${fontSizes}
`
