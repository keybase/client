  ...
  project.iconSets.forEach(set => {
    set.icons.forEach(icon => {
      ...
      if (name.startsWith('kb-')) {
        sizes[`fa-${name}`] = icon.grid
      }
    })
  })

  const json = JSON.stringify(sizes, null, 2).replace(/"/g, "'").replace('\n}', ',\n}\n')
  const data = `// @flow
// This file is GENERATED by npm run updated-fonts. DON'T hand edit

import type {IconType} from './icon'

const sizes: {[key: IconType]: number} = ${json}
export default sizes
`
  fs.writeFileSync('./shared/common-adapters/icon.font.size.js', data, 'utf8')
}
