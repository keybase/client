...
- (void)applicationDidFinishLaunching:(NSNotification *)notification {
  ...
  // Actually set these values instead of using registerDefaults because extensions have a different ~ (home).
  if (![userDefaults objectForKey:@"HomeDir"]) [userDefaults setObject:KBPath(@"~/Library/Application Support/Keybase/dev", NO, NO) forKey:@"HomeDir"];
  if (![userDefaults objectForKey:@"MountDir"]) [userDefaults setObject:KBPath(@"~/Keybase.dev", NO, NO) forKey:@"MountDir"];
  // TODO Fix defaults
