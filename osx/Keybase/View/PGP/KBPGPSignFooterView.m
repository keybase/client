...
- (void)viewInit {
  ...
  _clearSignButton = [KBButton buttonWithText:@"Clear Sign" style:KBButtonStyleCheckbox];
  _clearSignButton.state = NSOnState;
  [buttonsView addSubview:_clearSignButton];
  ...
  buttonsView.viewLayout = [YOLayout layoutWithLayoutBlock:^CGSize(id<YOLayout> layout, CGSize size) {
    ...
    CGFloat x = 20;
    x += [layout sizeToFitHorizontalInFrame:CGRectMake(x, y, 0, buttonSize.height) view:yself.clearSignButton].size.width + 20;
