...
def getPackagesToTest() {
  ...
  dir('go') {
    if (env.CHANGE_TARGET) {
      ...
      def diffPackageList = sh(returnStdout: true, script: "bash -c \"set -o pipefail; git merge-tree $(git merge-base HEAD ${BASE_COMMIT_HASH}) ${BASE_COMMIT_HASH} HEAD | grep '[0-9]\\+\\s[0-9a-f]\\{40\\}' | awk '{print $4}' | grep '^go\\/' | sed 's/^\\(.*\\)\\/[^\\/]*\$/github.com\\/keybase\\/client\\/\\1/' | sort | uniq\"").trim().split()
