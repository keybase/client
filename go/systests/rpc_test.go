...
func testIdentifyLite(t *testing.T) {
	...
	t.Logf("make a public implicit team")
	iTeamPubCreateName := iTeamCreateName
	iTeamPubID, _, err := teams.LookupOrCreateImplicitTeam(context.TODO(), g, iTeamPubCreateName, true /*public*/)
	iTeamPubImpName := getTeamName(iTeamPubID)
	require.NoError(t, err)

	require.False(t, iTeamID.Eq(iTeamPubID), "implicit teams must have different IDs")

	cli, err := client.GetIdentifyClient(g)
	require.NoError(t, err, "failed to get new identifyclient")

	// "__keybase_implicit_team__bbbb26367512347c55154f14d436fb8b" -> "bbbb26367512347c55154f14d436fb8b"
	impNamePart := func(impName keybase1.TeamName) string {
		re := regexp.MustCompile(fmt.Sprintf("^%s([0-9a-f]{%d})$", keybase1.ImplicitTeamPrefix, keybase1.ImplicitSuffixLengthBytes*2))
		matches := re.FindStringSubmatch(impName.String())
		require.Len(t, matches, 2, "bad iteam name: %v", impName.String())
		return matches[1]
	}
	...
	var units = []struct {
	...
	}{
		{
			...
			assertion: "itid:" + impNamePart(iTeamImpName),
			resID:     iTeamID,
			resName:   "private/" + iTeamCreateName,
		}, {
			assertion: "itid:" + impNamePart(iTeamPubImpName),
			resID:     iTeamPubID,
			resName:   "public/" + iTeamPubCreateName,
		},
	}
	...
	// test invalid itid format
	_, err = cli.IdentifyLite(context.Background(), keybase1.IdentifyLiteArg{Assertion: "itid:xxx"})
	require.Error(t, err)
	require.Contains(t, err.Error(), "team names must be")

	// test team read error
	assertions = []string{"team:jwkj22111z", "itid:bbdd26367512311115154f14d436fb8b"}
	for _, assertion := range assertions {
