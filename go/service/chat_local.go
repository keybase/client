...
func (h *chatLocalHandler) GetMessagesLocal(ctx context.Context, arg chat1.GetMessagesLocalArg) (res chat1.GetMessagesLocalRes, err error) {
	...
	var rlimptr []*chat1.RateLimit
	conv, err := chat.ConversationMetadata(ctx, h.remoteClient(), arg.ConversationID, &rlimptr)
	if err != nil {
		return deflt, err
	}
	for _, r := range rlimptr {
		rlimits = append(rlimits, *r)
	}
