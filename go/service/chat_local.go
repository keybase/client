...
func (h *chatLocalHandler) GetMessagesLocal(ctx context.Context, arg chat1.GetMessagesLocalArg) (res chat1.GetMessagesLocalRes, err error) {
	...
	uid := h.G().Env.GetUID()
	conv, rl, err := chat.ConversationMetadata(ctx, h.G().InboxSource, uid.ToBytes(), arg.ConversationID)
	if err != nil {
		return deflt, err
	}
	if rl != nil {
	...
	messages, err := h.boxer.UnboxMessages(ctx, boxed.Msgs, conv.Info.FinalizeInfo)
	if err != nil {
		return deflt, err
	}
