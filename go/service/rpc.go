...
func newConnTransport(g *libkb.GlobalContext) *connTransport {
	return &connTransport{Contextified: libkb.NewContextified(g)}
}

func (t *connTransport) Dial(context.Context) (rpc.Transporter, error) {
	var err error
	t.conn, err = net.Dial("tcp", t.G().Env.GetGregorURI())
	if err != nil {
		return nil, err
	}
