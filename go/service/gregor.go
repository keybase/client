...
func (g *gregorHandler) GetClient() chat1.RemoteInterface {
	ctx := libkb.WithLogTag(context.Background(), "GRGRCLI")
	cli := g.getRPCCli()
	if g.IsShutdown() || cli == nil {
		select {
		case <-g.connectHappened:
			cli = g.getRPCCli()
			if g.IsShutdown() || cli == nil {
				g.chatLog.Debug(ctx, "GetClient: connectHappened, but still shutdown, using OfflineClient for chat1.RemoteClient")
				return chat1.RemoteClient{Cli: chat.OfflineClient{}}

			}
			g.chatLog.Debug(ctx, "GetClient: successfully waited for connection")
			return chat1.RemoteClient{Cli: chat.NewRemoteClient(g.G(), cli)}
		case <-time.After(GregorGetClientTimeout):
			g.chatLog.Debug(ctx, "GetClient: shutdown, using OfflineClient for chat1.RemoteClient (waited %s for connectHappened)", GregorGetClientTimeout)
			return chat1.RemoteClient{Cli: chat.OfflineClient{}}
		}
	}
	g.chatLog.Debug(ctx, "GetClient: not shutdown, making new remote client")
	return chat1.RemoteClient{Cli: chat.NewRemoteClient(g.G(), cli)}
}
...
// OnConnect passes the ctx it derives for its connection, which Shutdown
// cancels under connMutex; checking and installing under connMutex too means
// Reset, which drops the client after its Shutdown, never runs between the
// two.
...
func (g *gregorHandler) connectNow(uri *rpc.FMPURI) (err error) {
	...
	handler := &gregorConnHandler{gregorHandler: g, connCtx: connCtx}
	var conn *rpc.Connection
	if uri.UseTLS() {
		conn, err = g.connectTLS(ctx, uri, handler)
		if err != nil {
			connCancel()
			return err
		}
	} else {
		conn = g.connectNoTLS(ctx, uri, handler)
	}
	g.conn = conn
	g.connCtx, g.connCancel = connCtx, connCancel
...
func (g *gregorHandler) PushHandler(handler libkb.GregorInBandMessageHandler) {
	ctx := libkb.WithLogTag(context.Background(), "GRGRPUSH")
	defer g.chatLog.Trace(ctx, nil, "PushHandler")()

	g.Debug(ctx, "pushing inband handler %s to position %d", handler.Name(), len(g.ibmHandlers))
	...
	if g.IsConnected() && cli != nil {
		if _, err := g.replayInBandMessages(ctx, gregor1.IncomingClient{Cli: cli},
			time.Time{}, handler); err != nil {
			g.Errorf(ctx, "replayInBandMessages on PushHandler failed: %s", err)
		}

		if g.badger != nil {
			s, err := g.getState(ctx)
			if err != nil {
				g.Warning(ctx, "Cannot get state in PushHandler: %s", err)
				return
			}
			g.badger.PushState(ctx, s)
		}
	}
}
...
func (g *gregorHandler) pushStateOnce(r keybase1.PushReason) {
	ctx := libkb.WithLogTag(context.Background(), "GRGRSTATE")
	s, err := g.getState(ctx)
	if err != nil {
		g.Warning(ctx, "Cannot push state in firehose handler: %s", err)
		return
	}
	g.iterateOverFirehoseHandlers(func(h libkb.GregorFirehoseHandler) {
		g.Debug(ctx, "pushState: pushing state with %d items", len(s.Items_))
		h.PushState(s, r)
	})
...
func (g *gregorHandler) syncReplayThread() {
	for rarg := range g.replayCh {
		...
		now := time.Now()
		// A logout can land after serverSync queued this, and the replay's
		// ctx is not cancelled by it; don't run the handlers for a user who
		// is no longer logged in.
		if !rarg.uid.Eq(g.currentUID()) {
			g.Debug(rarg.ctx, "serverSync: replayThread: skipping, %s is no longer logged in", rarg.uid)
...
func (g *gregorHandler) serverSync(ctx context.Context,
	...
	g.replayCh <- replayThreadArg{
		...
		uid: gregor1.UID(gcli.User.Bytes()),
	}
...
// onConnectCtx returns the ctx OnConnect runs under, or ErrDuplicateConnection
// if conn is not the current connection. The rpc library's own ctx is not
// enough: it cancels only the reconnect loop running when the connection is
// shut down, and any later call on that connection starts a new loop, and so
// a new OnConnect, under a ctx nothing cancels. The returned ctx is cancelled
// by conn's Shutdown, synchronously under connMutex, as well as by the rpc
// library. The rpc library's ctx carries no values, so none are lost.
func (g *gregorHandler) onConnectCtx(ctx context.Context, conn *rpc.Connection) (context.Context, context.CancelFunc, error) {
	g.connMutex.Lock()
	defer g.connMutex.Unlock()
	if conn == nil || conn != g.conn {
		return nil, nil, chat.ErrDuplicateConnection
	}
	res, cancel := context.WithCancel(g.connCtx)
...
// gregord
func (g *gregorHandler) OnConnect(rpcCtx context.Context, conn *rpc.Connection,
	cli rpc.GenericClient, srv *rpc.Server,
) (err error) {
	ctx, cancel, err := g.onConnectCtx(rpcCtx, conn)
	if err != nil {
		g.chatLog.Debug(libkb.WithLogTag(rpcCtx, "GRGRONCONN"), "aborting, not the current connection")
		return err
	}
	defer cancel()
...
func (g *gregorHandler) onConnectSynced(ctx context.Context, chatCli chat1.RemoteInterface,
	...
	// It runs outside the gate, which holds off every connect and disconnect,
	// to keep the gate's holds short: it writes storage and notifies. The
	// Syncer ignores a cancelled ctx, and Shutdown marks it disconnected after
	// cancelling.
...
// gregorConnHandler is the rpc.ConnectionHandler for one connection. The rpc
// library can keep calling a connection's handler after Shutdown: any call on
// a shut-down connection starts a new reconnect loop, which reports
// OnDisconnected. connCtx, which Shutdown cancels under connMutex, tells those
// apart from events for the live connection.
type gregorConnHandler struct {
	*gregorHandler
	connCtx context.Context
}

var _ rpc.ConnectionHandler = (*gregorConnHandler)(nil)

func (h *gregorConnHandler) OnConnectError(err error, reconnectThrottleDuration time.Duration) {
	ctx := libkb.WithLogTag(context.Background(), "GRGRONERR")
	defer h.chatLog.Trace(ctx, nil, "OnConnectError")()
	h.chatLog.Debug(ctx, "OnConnectError: err: %s, reconnect throttle duration: %s", err,
		reconnectThrottleDuration)

	// Check reachability here to see the nature of our offline status
	go func() {
		if h.reachability != nil && !h.isReachable(ctx, h.connCtx) {
			h.reachability.setReachability(keybase1.Reachability{
...
func (h *gregorConnHandler) OnDisconnected(ctx context.Context, status rpc.DisconnectStatus) {
	ctx = libkb.WithLogTag(ctx, "GRGRONDISC")
	// Under connMutex, so no newer connection can be up while this runs.
	h.connMutex.Lock()
	if h.connCtx.Err() != nil {
		h.connMutex.Unlock()
		h.chatLog.Debug(ctx, "disconnected: %v, ignoring, not the current connection", status)
		return
	}
	h.chatLog.Debug(ctx, "disconnected: %v", status)
	h.setConnectedAt(time.Time{})
	...
	go func() {
		if h.reachability != nil && status != rpc.StartingFirstConnection && !h.isReachable(ctx, h.connCtx) {
			h.reachability.setReachability(keybase1.Reachability{
				Reachable: keybase1.Reachable_NO,
			})
		}
...
func (g *gregorHandler) broadcastMessageHandler() {
	for {
		ctx := libkb.WithLogTag(context.Background(), "GRGRBCAST")
		m := <-g.broadcastCh
		...
		if err != nil {
			g.Debug(ctx, "broadcast error: %v", err)
		}
...
func (g *gregorHandler) handleOutOfBandMessage(ctx context.Context, obm gregor.OutOfBandMessage) error {
	...
	if tmp, ok := obm.(gregor1.OutOfBandMessage); ok {
		...
		g.Warning(ctx, "Got non-exportable out-of-band message")
	}
	...
	case "internal.reconnect":
		g.Debug(ctx, "reconnected to push server")
		return nil
	default:
...
func (g *gregorHandler) Shutdown(ctx context.Context) {
	...
	// Syncer.Connected from it either lands before this and is overwritten,
...
func (g *gregorHandler) loggedIn(ctx context.Context) (uid keybase1.UID, did keybase1.DeviceID, token string, nist *libkb.NIST, res loggedInRes) {
	...
	if nist == nil {
		g.Debug(ctx, "gregorHandler: no NIST for login; user isn't logged in")
		return uid, did, token, nil, loggedInNo
	}
	if err != nil {
		g.Debug(ctx, "gregorHandler: error in generating NIST: %s", err.Error())
		return uid, did, token, nil, loggedInMaybe
	}
...
// isReachable dials gregord, and on failure reconnects the connection whose
// ctx is connCtx.
func (g *gregorHandler) isReachable(ctx context.Context, connCtx context.Context) bool {
	...
	if err != nil {
		...
		g.connGate.requestReconnect(ctx, connCtx)
		return false
	}
...
// Reconnect drops the current connection and connects again when the app
// state allows it, without waiting for either. It does nothing if that
// connection has been replaced by the time the reconnect runs.
func (g *gregorHandler) Reconnect(ctx context.Context) {
	g.connGate.requestReconnect(ctx, g.currentConnCtx())
}

// currentConnCtx is the ctx of the current connection, nil if there is none.
func (g *gregorHandler) currentConnCtx() context.Context {
	g.connMutex.Lock()
	defer g.connMutex.Unlock()
	return g.connCtx
}
...
func (g *gregorHandler) pingOnce(ctx context.Context, conn context.Context, id []byte) {
	g.connMutex.Lock()
	pingCli := g.pingCli
	g.connMutex.Unlock()
	if pingCli == nil {
		g.chatLog.Debug(ctx, "ping loop: id: %x no connection, skipping ping", id)
		return
	}
	// Buffered so the goroutine never blocks after pingOnce has returned on
	// conn's shutdown.
	...
	go func(ctx context.Context) {
		...
		if g.IsConnected() {
			...
			_, err = gregor1.IncomingClient{Cli: pingCli}.Ping(timeoutCtx)
			timeoutCancel()
		} else {
			...
			_, err = gregor1.IncomingClient{Cli: pingCli}.Ping(ctx)
	...
	if err != nil {
		...
		if errors.Is(err, context.DeadlineExceeded) {
			...
			g.connGate.requestReconnect(ctx, conn)
		}
	}
}

// pingLoop runs until conn, the ctx of the connection it was started for, is
// done.
func (g *gregorHandler) pingLoop(ctx context.Context, conn context.Context) {
	...
	for {
		pingCtx, pingCancel := context.WithCancel(libkb.CopyTagsToBackground(ctx))
		select {
		case <-g.forcePingCh:
			g.chatLog.Debug(pingCtx, "ping loop: forced attempt")
			g.pingOnce(pingCtx, conn, id)
		case <-ticker.C:
			g.pingOnce(pingCtx, conn, id)
		case <-conn.Done():
...
func (g *gregorHandler) connectTLS(ctx context.Context, uri *rpc.FMPURI, handler rpc.ConnectionHandler) (*rpc.Connection, error) {
	...
	return rpc.NewTLSConnectionWithDialable(rpc.NewFixedRemote(uri.HostPort),
		...
		handler, libkb.NewRPCLogFactory(g.G().ExternalG()),
...
func (g *gregorHandler) connectNoTLS(ctx context.Context, uri *rpc.FMPURI, handler rpc.ConnectionHandler) *rpc.Connection {
	...
	return rpc.NewConnectionWithTransport(handler, t,
