...
func TestUnzipFileModTime(t *testing.T) {
	...
	assert.GreaterOrEqual(t, diffDir, time.Duration(0), "now=%s, dirtime=%s", now, dirMod)
	...
	assert.GreaterOrEqual(t, diffFile, time.Duration(0), "now=%s, filetime=%s", now, fileMod)
}
