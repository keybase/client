...
func (c *CmdCtlInit) RunEnv() error {
	...
	s.WriteString("# Autogenerated by `keybase ctl init`; do not edit.\n")
	s.WriteString("# Only used when running Keybase via systemd user manager.\n")
	s.WriteString(fmt.Sprintf("# To override individual variables, write to %s\n", overrideEnvfileName))
	for _, env := range envs {
		...
		if env.Unset {
			s.WriteString(fmt.Sprintf("# %s (unset)\n", env.Name))
		} else {
	...
	if c.DryRun {
		...
		_, err = envfile.WriteString(s.String())
		if err != nil {
			return err
		}
	}
