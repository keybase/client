...
func NewProveUIProtocol(ui ProveUI) rpc2.Protocol {
	return keybase1.ProveUiProtocol(&ProveUIServer{ui})
}

func (p *ProveUIServer) PromptOverwrite(arg keybase1.PromptOverwriteArg) (bool, error) {
	return p.eng.PromptOverwrite(arg.Account, arg.Typ)
}
func (p *ProveUIServer) PromptUsername(arg keybase1.PromptUsernameArg) (string, error) {
	return p.eng.PromptUsername(arg.Prompt, libkb.ImportStatusAsError(arg.PrevError))
}
func (p *ProveUIServer) OutputPrechecks(arg keybase1.OutputPrechecksArg) error {
	p.eng.OutputPrechecks(arg.Text)
	return nil
}
func (p *ProveUIServer) PreProofWarning(arg keybase1.PreProofWarningArg) (bool, error) {
	return p.eng.PreProofWarning(arg.Text)
}
func (p *ProveUIServer) OutputInstructions(arg keybase1.OutputInstructionsArg) error {
	return p.eng.OutputInstructions(arg.Instructions, arg.Proof)
}
func (p *ProveUIServer) OkToCheck(arg keybase1.OkToCheckArg) (bool, error) {
	return p.eng.OkToCheck(arg.Name, arg.Attempt)
}
func (p *ProveUIServer) DisplayRecheckWarning(arg keybase1.DisplayRecheckWarningArg) error {
...
func (v *CmdProve) RunClient() (err error) {
	...
	if cli, err = GetProveClient(); err != nil {
		...
		err = cli.Prove(keybase1.ProveArg{
			Username: v.username,
			Service:  v.service,
			Force:    v.force,
		})
