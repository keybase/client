...
func newKBPConfigEditorWithPrompter(kbpConfigDir string, p prompter) (
	*kbpConfigEditor, error) {
	...
	case err == nil:
		...
		switch cfg.Version() {
		case libpagesconfig.Version1:
			editor.kbpConfig = cfg.(*libpagesconfig.V1)
	...
	case os.IsNotExist(err):
		editor.kbpConfig = libpagesconfig.DefaultV1()
	default:
		return nil, fmt.Errorf(
...
func (e *kbpConfigEditor) setUser(username string, isAdd bool) error {
	...
	hashed, err := libpagesconfig.GenerateSHA256PasswordHash(password)
	if err != nil {
		return err
	}
...
func (e *kbpConfigEditor) setAnonymousPermission(
	...
	if e.kbpConfig.ACLs == nil {
		e.kbpConfig.ACLs = make(map[string]libpagesconfig.AccessControlV1)
	}
