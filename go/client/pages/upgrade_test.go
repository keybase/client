...
func TestUpgrade(t *testing.T) {
	...
	kbpConfigPath := filepath.Join(configDir, libpagesconfig.DefaultConfigFilename)

	t.Logf("creating config with bcrypt pass at %s", kbpConfigPath)

	v1 := libpagesconfig.DefaultV1()
	...
	cfg, err := libpagesconfig.ParseConfig(f2)
	require.NoError(t, err)
	require.Equal(t, libpagesconfig.Version1, cfg.Version())
	v1 = cfg.(*libpagesconfig.V1)
