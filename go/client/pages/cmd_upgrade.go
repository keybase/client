...
import (
	...
	"github.com/keybase/client/go/libpagesconfig"
	"github.com/keybase/client/go/minterm"
)

func migrateUserToSHA256Hash(p prompter, username string, oldConfig libpagesconfig.Config) (sha256Hash string, err error) {
	for { // loop until user ctrl-C the command
		...
		if oldConfig.Authenticate(context.Background(), username, password) {
			return libpagesconfig.GenerateSHA256PasswordHash(password)
		}
...
func upgradeToSHA256WithPrompter(kbpConfigDir string, prompter prompter) (err error) {
	...
	if cfg.Version() != libpagesconfig.Version1 {
		return fmt.Errorf(
			"unsupported config version %s", cfg.Version())
	}

	oldConfig := cfg.(*libpagesconfig.V1)
	...
	newConfig := libpagesconfig.DefaultV1()
	for p, acl := range oldConfig.ACLs {
		if newConfig.ACLs == nil {
			newConfig.ACLs = make(map[string]libpagesconfig.AccessControlV1)
		}
