...
type kbpConfigEditor struct {
	...
	kbpConfig         *libpagesconfig.V1
	originalConfigStr string
	prompter          prompter
}

func readConfigAndClose(from io.ReadCloser) (
	cfg libpagesconfig.Config, str string, err error) {
	defer from.Close()
	buf := &bytes.Buffer{}
	if cfg, err = libpagesconfig.ParseConfig(io.TeeReader(from, buf)); err != nil {
		return nil, "", err
	}
...
func kbpConfigPath(kbpConfigDir string) (path string, err error) {
	...
	return filepath.Join(kbpConfigDir, libpagesconfig.DefaultConfigFilename), nil
}
...
func confirmAndWrite(
	originalConfigStr string,
	newConfig libpagesconfig.Config,
	configPath string,
