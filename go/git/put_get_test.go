...
func TestPutAndGet(t *testing.T) {
	...
	err = teams.CreateRootTeam(context.Background(), tc.G, teamName1, nil)
	require.NoError(t, err)
	team1, err := tc.G.GetTeamLoader().Load(context.Background(), keybase1.LoadTeamArg{Name: teamName1})
	require.NoError(t, err)

	teamName2 := u.Username + "t2"
	err = teams.CreateRootTeam(context.Background(), tc.G, teamName2, nil)
	require.NoError(t, err)
...
func TestPutAndGetWritersCantDelete(t *testing.T) {
	...
	err = teams.CreateRootTeam(context.Background(), tc.G, teamName, nil)
	require.NoError(t, err)
