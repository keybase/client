...
func checkRegistryKeybaseDokan(productIDKey string, log Log) (bool, error) {
	k, err := registry.OpenKey(registry.LOCAL_MACHINE, `Computer\HKEY_CURRENT_USER\Software\Keybase\Keybase\`+productIDKey, registry.QUERY_VALUE|registry.WOW64_64KEY)
	defer util.Close(k)
	if err != nil {
		return false, err
	}
	...
	defer util.Close(k2)
	if err == nil {
		log.Info("CheckRegistryKeybaseDokan: Found %s", productID)
		return true, nil
	}
	return false, err
}
