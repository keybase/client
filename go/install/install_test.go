...
func TestLastModifiedMatchingFile(t *testing.T) {
	...
	err = os.WriteFile(filepath.Join(tmpdir, fmt.Sprintf("first%sfile.txt", nameMatch)), []byte(unmatchingContent), 0o644)
	require.NoError(t, err)
	err = os.WriteFile(filepath.Join(tmpdir, "secondfile.txt"), []byte(matchingContent), 0o644)
	require.NoError(t, err)
	...
	err = os.WriteFile(fullPath, []byte(matchingContent), 0o644)
	require.NoError(t, err)
	match, err = LastModifiedMatchingFile(filePattern, contentMatch)
	require.NoError(t, err)
	...
	err = os.WriteFile(filepath.Join(tmpdir, fmt.Sprintf("fifth%sfile.txt", nameMatch)), []byte(unmatchingContent), 0o644)
	require.NoError(t, err)
	err = os.WriteFile(filepath.Join(tmpdir, "sixthfile.txt"), []byte(matchingContent), 0o644)
	require.NoError(t, err)
	match, err = LastModifiedMatchingFile(filePattern, contentMatch)
	require.NoError(t, err)
	require.NotNil(t, match)
