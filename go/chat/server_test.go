...
func TestChatSrvGracefulUnboxing(t *testing.T) {
	runWithMemberTypes(t, func(mt chat1.ConversationMembersType) {
		...
		tc := ctc.world.Tcs[users[0].Username]
		uid := users[0].User.GetUID().ToBytes()
		require.NoError(t, tc.Context().ConvSource.Clear(context.TODO(), created.Id, uid, types.ClearOpts{}))
...
func TestChatSrvGetThreadNonblockSupersedes(t *testing.T) {
	runWithMemberTypes(t, func(mt chat1.ConversationMembersType) {
		...
		require.NoError(t, cs.Clear(context.TODO(), conv.Id, uid, types.ClearOpts{}))
		tc := ctc.world.Tcs[users[0].Username]
...
func TestChatSrvGetUnreadLine(t *testing.T) {
	runWithMemberTypes(t, func(mt chat1.ConversationMembersType) {
		...
		assertUnreadline := func(ctx context.Context, g *globals.ChatContext, user *kbtest.FakeUser,
			...
			for i := 0; i < 1; i++ {
				if i == 0 {
					require.NoError(t, g.ConvSource.Clear(ctx, conv.Id, user.GetUID().ToBytes(), types.ClearOpts{}))
				}
...
func TestChatSrvGetThreadNonblockError(t *testing.T) {
	runWithMemberTypes(t, func(mt chat1.ConversationMembersType) {
		...
		require.NoError(t,
			ctc.world.Tcs[users[0].Username].ChatG.ConvSource.Clear(context.TODO(), conv.Id, uid, types.ClearOpts{}))
...
func TestChatSrvTLFConversationsLocal(t *testing.T) {
	runWithMemberTypes(t, func(mt chat1.ConversationMembersType) {
		...
		for i, user := range users {
			getTLFRes, err = ctc.as(t, user).chatLocalHandler().GetTLFConversationsLocal(
				ctc.as(t, user).startCtx,
				chat1.GetTLFConversationsLocalArg{
					TlfName:     conv.TlfName,
					TopicType:   chat1.TopicType_CHAT,
					MembersType: chat1.ConversationMembersType_TEAM,
...
func TestChatSrvUserResetAndDeleted(t *testing.T) {
	runWithMemberTypes(t, func(mt chat1.ConversationMembersType) {
		...
		uid0 := users[0].User.GetUID().ToBytes()
...
func TestTeamBotChannelMembership(t *testing.T) {
	runWithMemberTypes(t, func(mt chat1.ConversationMembersType) {
		...
		for _, user := range users {
			...
			uids, err := g.ParticipantsSource.Get(context.TODO(), user.GetUID().ToBytes(),
