...
func (g *PushHandler) TlfResolve(ctx context.Context, m gregor.OutOfBandMessage) (err error) {
	...
	cb := g.orderer.WaitForTurn(ctx, uid, update.InboxVers)
	f := func(ctx context.Context) (err error) {
		defer g.Trace(ctx, func() error { return err }, "TlfResolve(goroutine)")()
		<-cb
		g.Lock()
		defer g.Unlock()
		defer g.orderer.CompleteTurn(ctx, uid, update.InboxVers)
...
func (g *PushHandler) Activity(ctx context.Context, m gregor.OutOfBandMessage) (err error) {
	...
	f := func(ctx context.Context) (err error) {
		defer g.Trace(ctx, func() error { return err }, "Activity(goroutine)")()
		<-cb
		g.Lock()
		defer g.Unlock()
		...
		case types.ActionNewMessage:
			...
			var unboxInfo types.UnboxConversationInfo
			if conv == nil {
				unboxInfo = types.NewEmptyRemoteConversation(nm.ConvID)
			} else {
				unboxInfo = conv
			}
			decmsg, appended, pushErr := g.G().ConvSource.Push(ctx, unboxInfo, gregor1.UID(uid), nm.Message)
			if pushErr != nil {
...
func (g *PushHandler) SubteamRename(ctx context.Context, m gregor.OutOfBandMessage) (err error) {
	...
	cb := g.orderer.WaitForTurn(ctx, uid, update.InboxVers)
	f := func(ctx context.Context) (err error) {
		defer g.Trace(ctx, func() error { return err }, "SubteamRename(goroutine)")()
		<-cb
		g.Lock()
		defer g.Unlock()
		defer g.orderer.CompleteTurn(ctx, uid, update.InboxVers)
