...
type StorageEngine interface {
	ExecTransaction(ctx logger.CtxAndLogger, txFn func(logger.CtxAndLogger, Transaction) error) error

	// StoreKVPairs stores the []KeyEncodedValuePair in the tree.
	StoreKEVPairs(logger.CtxAndLogger, Transaction, Seqno, []KeyEncodedValuePair) error

	StoreNode(logger.CtxAndLogger, Transaction, Seqno, *Position, Hash) error

	StoreNodes(logger.CtxAndLogger, Transaction, Seqno, []PositionHashPair) error

	StoreRootMetadata(logger.CtxAndLogger, Transaction, RootMetadata) error
	...
	LookupLatestRoot(logger.CtxAndLogger, Transaction) (Seqno, RootMetadata, error)

	// If there is no root for the specified Seqno, an InvalidSeqnoError is returned.
	LookupRoot(logger.CtxAndLogger, Transaction, Seqno) (RootMetadata, error)
	...
	LookupNode(c logger.CtxAndLogger, t Transaction, s Seqno, p *Position) (Hash, error)
	...
	LookupNodes(c logger.CtxAndLogger, t Transaction, s Seqno, positions []Position) ([]PositionHashPair, error)
	...
	LookupKEVPair(c logger.CtxAndLogger, t Transaction, s Seqno, k Key) (val EncodedValue, s1 Seqno, err error)
	...
	LookupKEVPairsUnderPosition(ctx logger.CtxAndLogger, t Transaction, s Seqno, p *Position) ([]KeyEncodedValuePair, []Seqno, error)
}
...
type StorageEngineWithBlinding interface {
	...
	StoreMasterSecret(ctx logger.CtxAndLogger, t Transaction, s Seqno, ms MasterSecret) error
	LookupMasterSecrets(ctx logger.CtxAndLogger, t Transaction, s []Seqno) (map[Seqno]MasterSecret, error)
}
