...
func TestMerkleRootPayloadUnmarshalWithSkips(t *testing.T) {
	...
	require.NoError(t, err)
	require.True(t, hashEq(h, r.Body.Skips[227711]),
		"should have had hash equality")
}
...
func TestMerkleRootPayloadUnmarshalWithoutSkips(t *testing.T) {
	...
	err := json.Unmarshal([]byte(raw), &r)
	require.NoError(t, err)
	require.Nil(t, r.Body.Skips,
		"expected an empty skip table")
}
...
func TestMerkleSkipVectors(t *testing.T) {
	...
	for i, v := range merkleSkipTestVectors {
		...
		if v.e == merkleErrorNone {
			require.NoError(t, err, "Expected no error in test %s, but got %s", v.name, err)
			continue
		}
		me, ok := err.(MerkleClientError)
		require.True(t, ok,
			"Got an unexpected error type in test %s: %T (%v)", v.name, err, err)
		require.Equal(t, v.e, me.t, "Got wrong error type in test %s: %v != %v", v.name, me.t, v.e)
	}
}
...
func TestComputeSetBitsBigEndian(t *testing.T) {
	...
	for _, test := range tests {
		...
		require.True(t, reflect.DeepEqual(got, test.expected),
			"Failed on input %d, expected %v, got %v.", test.x, test.expected, got)
	}
}
...
func TestComputeExpectedRootSkips(t *testing.T) {
	...
	for _, test := range tests {
		...
		require.True(t, reflect.DeepEqual(got, test.expected),
			"Failed on input (%d), expected %v, got %v.", test.root, test.expected, got)
	}
}
