...
func TestSecWordList(t *testing.T) {
	...
	require.Equal(t, 6, len(words), "# words = %d, expected 6", len(words))
}
...
func TestSecWordList128(t *testing.T) {
	...
	require.NoError(t, err)
	t.Logf("words: %v", words)
	require.Equal(t, 12, len(words), "# words = %d, expected 12", len(words))
}
...
func TestSecWordList144(t *testing.T) {
	...
	require.NoError(t, err)
	t.Logf("words: %v", words)
	require.Equal(t, 14, len(words), "# words = %d, expected 14", len(words))
}

func TestSecWordListConstants(t *testing.T) {
	words, err := SecWordList(PaperKeySecretEntropy + PaperKeyIDBits + PaperKeyVersionBits)
	require.NoError(t, err)
	require.Equal(t, PaperKeyWordCountMin, len(words), "paper key words for constants: %d, expected %d", len(words), PaperKeyWordCountMin)
}
