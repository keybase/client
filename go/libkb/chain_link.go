...
func (c *ChainLink) Unpack(trusted bool, selfUID keybase1.UID) (err error) {
	...
	if trusted {
		...
		if e2 == nil && b {
			c.sigVerified = true
			c.G().VDL.Log(VLog1, VTagSigChain, "| Link is marked as 'sig_verified'")
	...
	c.G().VDL.Log(VLog1, VTagSigChain, "| Unpacked Link %s", c.id)
...
func (c *ChainLink) PutSigCheckCache(cki *ComputedKeyInfos) {
	c.G().VDL.Log(VLog1, VTagSigChain, "Caching SigCheck for link %s:", c.id)
	c.sigVerified = true
	c.dirty = true
	c.cki = cki
	return
...
func (c *ChainLink) Store(g *GlobalContext) (didStore bool, err error) {
	...
	g.VDL.Log(VLog1, VTagSigChain, "| Store Link %s", c.id)
