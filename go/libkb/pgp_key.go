...
func NewPGPKeyBundle(g *GlobalContext, entity *openpgp.Entity) *PGPKeyBundle {
	return &PGPKeyBundle{Entity: entity, Contextified: NewContextified(g)}
}
...
func (k PGPKeyBundle) VerifyStringAndExtract(sig string) (msg []byte, id keybase1.SigID, err error) {
	...
	if ps, err = PGPOpenSig(sig); err != nil {
		...
		k.G().Log.Debug("Failing key: %s", k.ArmoredPublicKey)
		k.G().Log.Debug("Failing Sig: %s", sig)
		return
	}
	msg = ps.LiteralData
	id = ps.ID()
