...
func (e *SaltpackDecrypt) Run(m libkb.MetaContext) (err error) {
	...
	if decErr, ok := err.(libkb.DecryptionError); ok {
		...
		case saltpack.ErrNoSenderKey:
			err = libkb.DecryptionError{Cause: libkb.ErrorCause{Err: decErr.Cause.Err, StatusCode: libkb.SCDecryptionKeyNotFound}}
		case saltpack.ErrWrongMessageType:
			err = libkb.DecryptionError{Cause: libkb.ErrorCause{Err: decErr.Cause.Err, StatusCode: libkb.SCWrongType}}
		}
	}
