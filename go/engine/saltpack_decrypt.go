...
func (e *SaltpackDecrypt) Run(m libkb.MetaContext) (err error) {
	...
	if err == saltpack.ErrNoDecryptionKey {
		m.CDebugf("substituting saltpack.ErrNoDecryptionKey with more specific libkb.NoDecryptionKeyError")
		if e.arg.Opts.UsePaperKey {
			return libkb.NoDecryptionKeyError{Msg: "this message was not directly encrypted for the given paper key. In some cases, you might still be able to decrypt the message from a device provisioned with this key."}
		}
		err = libkb.NoDecryptionKeyError{Msg: "no suitable key found"}
	}
