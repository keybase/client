...
func (e *LoginProvision) device(ctx *Context) error {
	...
	provisionerType, err := ctx.ProvisionUI.ChooseDeviceType(ctx.NetContext(), arg)
	if err != nil {
		return err
	}
...
func (e *LoginProvision) deviceName(ctx *Context) (string, error) {
	...
	return ctx.ProvisionUI.PromptNewDeviceName(ctx.NetContext(), arg)
}
...
func (e *LoginProvision) promptEmailOrUsername(ctx *Context) (string, error) {
	...
	return ctx.LoginUI.GetEmailOrUsername(ctx.NetContext(), 0)
}
...
func (e *LoginProvision) chooseMethod(ctx *Context) (keybase1.ProvisionMethod, error) {
	...
	return ctx.ProvisionUI.ChooseProvisioningMethod(ctx.NetContext(), arg)
}
...
func (e *LoginProvision) selectGPGKey(ctx *Context) (fp *libkb.PGPFingerprint, err error) {
	...
	keyid, err := ctx.GPGUI.SelectKey(ctx.NetContext(), keybase1.SelectKeyArg{Keys: gks})
	if err != nil {
		return nil, err
	}
...
func (e *LoginProvision) displaySuccess(ctx *Context) error {
	...
	return ctx.ProvisionUI.ProvisioneeSuccess(ctx.NetContext(), sarg)
}
