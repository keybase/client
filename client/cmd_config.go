...
func (v *CmdConfig) Run() error {
	...
	if v.key != "" {
		if v.valueSet {
			...
			if s, isSet := cr.GetStringAtPath(v.key); is_set {
				fmt.Fprintf(v.writer, "%s: %s\n", v.key, s)
			} else if b, isSet := cr.GetBoolAtPath(v.key); is_set {
				fmt.Fprintf(v.writer, "%s: %t\n", v.key, b)
			} else if i, isSet := cr.GetIntAtPath(v.key); is_set {
				fmt.Fprintf(v.writer, "%s: %d\n", v.key, i)
			} else if isSet := cr.GetNullAtPath(v.key); is_set {
