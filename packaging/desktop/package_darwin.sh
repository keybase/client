...
saltpack_encrypted_icon="$client_dir/media/icons/saltpack-encrypted.icns"
saltpack_signed_icon="$client_dir/media/icons/saltpack-signed.icns"
...
installer_url="https://prerelease.keybase.io/darwin-package/KeybaseInstaller-1.1.80-darwin.tgz"
...
package_electron() {(
  ...
  yarn install --pure-lockfile
  yarn run package -- --appVersion="$app_version" --comment="$comment" --icon="$icon_path" --saltpackEncryptedIcon="$saltpack_encrypted_icon" --saltpackSignedIcon="$saltpack_signed_icon" --outDir="$build_dir"
