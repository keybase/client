...
if [ ! "$nowait" = "1" ]; then
  ...
  "$release_bin" wait-ci --repo="client" --commit=`git -C $client_dir log -1 --pretty=format:%h` --context="Jenkins - Linux" --context="Jenkins - Windows" --context="Jenkins - OS X" --context="ci/circleci"
