...
if [ ! "$nowait" = "1" ]; then
  ...
  "$release_bin" wait-ci --repo="client" --commit=`git -C $client_dir log -1 --pretty=format:%h` --context="client-osx-linux-only/label=linux" --context="client-osx-linux-only/label=windows" --context="client-osx-master-only/label=osx" --context="ci/circleci"
