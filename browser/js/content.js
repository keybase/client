...
function init() {
  chrome.storage.sync.get(function(options) {
    ...
    // Passive queries?
    if (options['profile-passive-queries'] === true && user) {
    ...
    if (options['reddit-thread-reply'] !== false) {
    ...
    if (options['profile-chat-buttons'] !== false && user) {
      // undefined defaults to true
...
function renderChatContact(el, user) {
  ...
  if (keybaseUsername) {
    ...
    iconSrc = bel`
      <img class="keybase-icon"
           ...
           srcset="${asset('images/icon-placeholder-avatar-32@2x.png')} 2x, ${asset(
      'images/icon-placeholder-avatar-32@3x.png'
    )} 3x"
           />
    `
  }
  morphdom(
    ...
    <div class="keybase-contact">
      <div>${iconSrc}</div>
      Encrypt to <a class="keybase-user ${
        user.origin
      }" href="${user.href()}" target="_blank">${user.display()}</a>
      ${queryStatus}
    </div>
  `
  )
  ...
  const f = bel`<form class="keybase-reply ${user.extraReplyCls}" action="#">
    <h3>
      <img src="${asset('images/icon-keybase-logo-16.png')}"
           srcset="${asset('images/icon-keybase-logo-16@2x.png')} 2x, ${asset(
    'images/icon-keybase-logo-16@3x.png'
  )} 3x"
           />
      ...
      <div class="keybase-body">
        <p>
          <img src="${asset('images/icon-fancy-chat-sent-desktop.png')}" style="width: 96px; height: 64px;"
               srcset="${asset('images/icon-fancy-chat-sent-desktop@2x.png')} 2x, ${asset(
      'images/icon-fancy-chat-sent-desktop@3x.png'
    )} 3x"
               />
        </p>
        <p class="keybase-success-message">Your message was sent! <br>
        You can continue the conversation in your Keybase app.</p>
        ${extraHTML}
        <p>
          <input type="button" class="keybase-close" value="OK" />
        </p>
      </div>
    </div>
  `,
