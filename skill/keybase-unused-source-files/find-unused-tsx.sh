...
find "$SHARED_ROOT" -type f \( -name "*.ts" -o -name "*.tsx" -o -name "*.js" -o -name "*.mts" \) \
  ...
  ! -path "*/dist/*" \
  ! -path "*/coverage-ts/*" \
  | xargs grep -hE "(from |import\(|import |require\()[\'\"](\.|@)" 2>/dev/null \
  > /tmp/tsx-check-imports.txt
...
check_tsx() {
  ...
  if [[ "$generic" != "$stem" ]]; then
    grep -qF "/${generic}'" /tmp/tsx-check-imports.txt && return 0
    grep -qF "/${generic}\"" /tmp/tsx-check-imports.txt && return 0
  fi
