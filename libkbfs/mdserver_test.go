...
func makeBRMDForTest(t *testing.T, crypto cryptoPure, id tlf.ID,
	h tlf.BareTlfHandle, revision MetadataRevision, uid keybase1.UID,
...
func TestMDServerRegisterForUpdate(t *testing.T) {
	...
	// Create first TLF.
	h1, err := tlf.MakeBareTlfHandle([]keybase1.UID{uid}, nil, nil, nil, nil)
	require.NoError(t, err)
	...
	// the first one.
	h2, err := tlf.MakeBareTlfHandle([]keybase1.UID{uid}, []keybase1.UID{keybase1.PUBLIC_UID}, nil, nil, nil)
	require.NoError(t, err)
