...
func TestCheckForHolesAndTruncate(t *testing.T) {
	// Make a mock block that believes itself to have three children.
	...
	for _, goodEx := range goodExamples {
		...
		if !reflect.DeepEqual(paths, newPaths) {
			t.Errorf("Paths incorrectly truncated from %v to %v",
				paths, newPaths)
		}
	}
	...
	for idx, badEx := range badExamples {
		...
		newPaths := checkForHolesAndTruncate(paths)
		if len(newPaths) != correctLengths[idx] {
			t.Errorf("Paths incorrectly truncated from %v to %v",
				paths, newPaths)
